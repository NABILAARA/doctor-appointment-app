...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const docsRes = await axios.get("http://localhost:8000/api/admin/getalldoctors", { 
          headers: { Authorization: `Bearer ${token}` } 
        });
        if (docsRes.data.success) setDoctors(docsRes.data.data);
        
        const usersRes = await axios.get("http://localhost:8000/api/admin/getallusers", { 
          headers: { Authorization: `Bearer ${token}` } 
        });
        if (usersRes.data.success) setUsers(usersRes.data.data);
        
        const aptRes = await axios.get("http://localhost:8000/api/admin/getallappointments", { 
          headers: { Authorization: `Bearer ${token}` } 
        });
  ...
  const handleApprove = async (doctorId, userId) => {
    const token = localStorage.getItem("token");
    try {
      await axios.post("http://localhost:8000/api/admin/approvedoctor", 
        { doctorId, userId }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="dashboard">
      ...
      <div className="main-content">
        ...
        <div className="stats-grid">
          <div className="stat-card"><div className="stat-value">{pending.length}</div><div>Pending</div></div>
          ...
          <div className="stat-card"><div className="stat-value">{appointments.length}</div><div>Appointments</div></div>
        </div>
        ...
        {activeTab === "doctors" && (
          <>
                ...
                <tbody>
                  {pending.map(doc => (
                    <tr key={doc._id}>
                      ...
                      <td>Rp {doc.fees}</td>
                      <td><button className="btn-primary" onClick={() => handleApprove(doc._id, doc.userId)}>Approve</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <h3 style={{ marginTop: "30px" }}>✅ Approved Doctors</h3>
            <div className="table-container">
              <table>
                <thead>
                  <tr>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u._id}>
                    ...
                    <td>{u.email}</td>
                    <td>{u.phone}</td>
                    <td>{u.type === "admin" ? "👑 Admin" : "👤 Patient"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
